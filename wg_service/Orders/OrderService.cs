...
namespace wg_service.Orders
{
	public class OrderService
	{
		...
		{
			var query = _context.t4_order.Include(e => e.Product).AsQueryable();
			if (userId.HasValue)
		...
		public async Task CreateOrder(int userId, string way, int pid)
		{
			...
			if (await _context.t4_order.AnyAsync(e => e.UserId == userId && e.ProductId == pid))
			...
			_context.t4_order.Add(new t4_order
			{
				...
				ProductId = pid,
				UserId = userId,
				Amt = way == "amt" ? product.Price : product.Invs,
				pay_way = way,
