...
namespace wg_service.Users
{
	public class InvitationService
	{
		...
		{
			var invInfo = await _context.t3_user_product_invitation.Include(e => e.t3_user_product_invitation_ip).FirstOrDefaultAsync(e => e.inv_code == invCode);
			if (invInfo != null)
			{
				var invIp = invInfo.t3_user_product_invitation_ip.FirstOrDefault(e => e.inv_ip == ip);
				if (invIp == null)
				{
					invInfo.t3_user_product_invitation_ip.Add(new t3_user_product_invitation_ip
					{
						count = 1,
						inv_ip = ip,
					});
